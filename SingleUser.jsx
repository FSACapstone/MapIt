              ...
              return (
                <div className="map-flex-inner" key={mapId}>
                  <Link to={`/map/${mapId}`}>
                    ...
                    <h2>{createdMaps[mapId].title}</h2>
                    <h3>
                      <Count of={db.collection('favoritedMaps').where('mapId', '==', mapId)} />{' '}
                      Likes
                    </h3>
                  </Link>
                  {signedInUser.uid === user.uid ? (
                    <div>
                      ...
                      <Button color="secondary" onClick={() => this.deleteMap(mapId)}>
                        Delete
                      </Button>
                    </div>
                  ) : null}
