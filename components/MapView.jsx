...
const Layer = ({ map }) => (
    <DataMap
        from={map.collection('places')}
        Render={({ position: { latitude: lat, longitude: lng } }) => (
...
const SearchBar = ({ from, style }) => (
    <DataMap
        from={from}
        Render={({ place }) => (
            <input style={style} placeholder="Search for a place..." />
        )}
    />
)
...
export default class Editor extends React.Component {
    ...
    mapWasClicked = ({ latLng }) => this.setState({ adding: latLng })
