        ...
        (() => {
          ...
          google.maps.event.addListener(marker, "click", function () {
            infowindow.setContent(
              ...
              placeName +
              "</strong><br>Address:" +
              placeAddress + '<br> From:' + map.title +
              "</div> "
            );
      ...
      .then(querySnapshot => {
        querySnapshot.forEach(map => {
          db
            ...
            .get()
            .then(querySnapshot => querySnapshot.forEach(nestMap => {
    ...
    return (<div>
      ...
      {this.state.checkedMaps.length
        ? <div>
          ...
          <button className="layered-save-button" onClick={this.onSaveClick}> Save Layered Map </button>
      ...
      {this.state.maps.length ? <div style={styles.block}> {this.state.maps.map(map => {
        return <p key={map.mid}><Checkbox className='margin-top-5' value={map.mid} onChange={this.onCheckClick}
